...
class InstitutModul(models.Model):

    #Nombre y descripcion del modelo
    _name = 'institut.modul'
    #Hereda de "base.archive" (el modelo abstracto creado antes)
    _inherit = ['base.archive']

    _description = 'Modul en el Insititut'

    _rec_name = 'nom'
 
    nom = fields.Char('Nom', required=True, index=True)
    cicle = fields.Many2one('institut.modul')
    alumnes = fields.Many2many('institut.alumne')
    professor = fields.Many2one('institut.professor')
